...
void BuildingSprite::draw_rally_point(Camera& camera) const
{
  if (this->building->rally_point == Vec2::zero)
    return ;
  const auto start_cam_pos = camera.world_to_camera_position(this->building->get_center_position());
  const sf::Vector2f start(start_cam_pos.x - camera.x,
                           start_cam_pos.y - camera.y);
  const auto end_cam_pos = camera.world_to_camera_position(this->building->rally_point);
...
bool BuildingSprite::is_in_screen(Camera& camera) const
{
  ...
  auto entpos = camera.world_to_camera_position(pos);
